// Geckodupe Info Page: D3 area charts (hardcoded benchmark data)

var INFO_COLORS = {
  primary: '#d8d23e',
  secondary: '#b0cc45',
  primaryStroke: 'rgba(168, 162, 48, 0.85)',
  secondaryStroke: 'rgba(138, 170, 56, 0.85)',
  axis: 'rgba(90, 87, 72, 0.45)',
  grid: 'rgba(90, 87, 72, 0.12)',
  text: '#5a5748',
  muted: 'rgba(90, 87, 72, 0.65)'
};
...
function appendGradients(svg) {
  ...
  defs.append('linearGradient').attr('id', 'area-grad-primary').attr('x1', '0').attr('y1', '0').attr('x2', '0').attr('y2', '1')
    .selectAll('stop').data([
      { offset: '0%', color: 'rgba(216, 210, 62, 0.38)' },
      { offset: '100%', color: 'rgba(216, 210, 62, 0.04)' }
    ]).enter().append('stop').attr('offset', function(d) { return d.offset; }).attr('stop-color', function(d) { return d.color; });
  defs.append('linearGradient').attr('id', 'area-grad-secondary').attr('x1', '0').attr('y1', '0').attr('x2', '0').attr('y2', '1')
    .selectAll('stop').data([
      { offset: '0%', color: 'rgba(176, 204, 69, 0.38)' },
      { offset: '100%', color: 'rgba(176, 204, 69, 0.04)' }
    ]).enter().append('stop').attr('offset', function(d) { return d.offset; }).attr('stop-color', function(d) { return d.color; });
}
...
function renderThroughputArea(rows, yKey, yFormat, legendLabel) {
  ...
  g.selectAll('.area-dot').data(data).enter().append('circle').attr('cx', function(d) { return x(d.x); }).attr('cy', function(d) { return y(d.y); }).attr('r', 3).attr('fill', '#f4f0e3').attr('stroke', INFO_COLORS.primaryStroke).attr('stroke-width', 1.5);
